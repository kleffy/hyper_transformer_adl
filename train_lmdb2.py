...
num_channels = 224
config_path = r'/vol/research/RobotFarming/Projects/hyper_transformer/configs/config_HSIT_enmap_ft_hyper_5.json'
...
__dataset__ = { "pavia_dataset": pavia_dataset, "botswana_dataset": botswana_dataset, "chikusei_dataset": chikusei_dataset, "botswana4_dataset": botswana4_dataset, "enmap_dataset": enmap_dataset}

# Parse the arguments
parser = argparse.ArgumentParser(description='PyTorch Training')
parser.add_argument('-c', '--config', default=config_path,type=str, help='Path to the config file')
parser.add_argument('-r', '--resume', default=best_pre_model_path, type=str, help='Path to the .pth model checkpoint to resume training')
...
keys = read_csv_keys(os.path.join(config["enmap_dataset"]["lmdb_save_dir"], config["enmap_dataset"]["csv_file_name"]), config["enmap_dataset"]["columns"][0])

train_keys, val_keys = train_test_split(keys, test_size=config["enmap_dataset"]["val_split"], random_state=42, shuffle=False)

print(f"Number of train set: {len(train_keys)} && Number of validation set: {len(val_keys)}")
train_loader = data.DataLoader(__dataset__[config["train_dataset"]](config,train_keys,is_train=True,want_DHP_MS_HR=config["is_DHP_MS"],), batch_size=config["train_batch_size"], num_workers=config["num_workers"], shuffle=True, pin_memory=False,)

test_loader = data.DataLoader(
                        __dataset__[config["train_dataset"]](config,val_keys,is_train=False,want_DHP_MS_HR=config["is_DHP_MS"],),
...
if config["optimizer"]["type"] == "SGD":
    optimizer = optim.SGD(model.parameters(), lr=config["optimizer"]["args"]["lr"], momentum = config["optimizer"]["args"]["momentum"], weight_decay= config["optimizer"]["args"]["weight_decay"])
