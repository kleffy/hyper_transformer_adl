...
# open the tif file
root_dir = r'/vol/research/RobotFarming/Projects/hyper_transformer/datasets/enmap_gdal/n1_l1'
for i, img in enumerate(os.listdir(root_dir)):
    ...
    # print(data.shape)
    # reshape the data array to a 3D array with 224 channels
    data = np.transpose(data, (1, 2, 0))

    # display a random channel using matplotlib
    plt.imshow(data[:, :, 0], cmap='gist_rainbow_r')
    plt.show()
    if (i + 1) % 3 == 0:
        break
...
# 'ocean_r', 'pink', 'pink_r', 'plasma', 'plasma_r', 'prism', 'prism_r', 'rainbow', 'rainbow_r', 'seismic', 'seismic_r', 'spring', 'spring_r', 
